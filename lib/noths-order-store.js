...
NothsOrderStore.prototype.query = function(query, callback) {
    ...
    var findOrders = function(err, orderIds) {
        ...
        async.map(orderIds, function(orderId, callback) {
            ...
            callback(new NothsOrderCollection(orders));
        });
    };
...
NothsOrderStore.prototype.stats = function(callback) {
    ...
    this.query({
        ...
        stats.todaysUkTtv = orders.ttv();
        stats.todaysUkOrderCount = orders.count();

        self.query({
            origin: 'de',
            date: new Date()
        }, function(orders) {
            stats.todaysDeTtv = orders.ttv();
            stats.todaysDeOrderCount = orders.count();
